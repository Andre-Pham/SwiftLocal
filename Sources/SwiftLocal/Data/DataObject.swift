...
public class DataObject {
    ...
    public func get(_ key: String, legacyKeys: [String] = []) -> [String] {
        ...
        assert(retrieval != nil, "Failed to restore attribute '\(key)' to object '\(self.objectName)'")
        let result = (retrieval ?? []).compactMap(\.string)
        assert(
            retrieval?.count == result.count,
            "JSON array came with \(retrieval?.count ?? -1) elements, but only \(result.count) could be restored"
        )
        return result
    }
    ...
    public func get(_ key: String, legacyKeys: [String] = []) -> [Int] {
        ...
        assert(retrieval != nil, "Failed to restore attribute '\(key)' to object '\(self.objectName)'")
        let result = (retrieval ?? []).compactMap(\.int)
        assert(
            retrieval?.count == result.count,
            "JSON array came with \(retrieval?.count ?? -1) elements, but only \(result.count) could be restored"
        )
    ...
    public func get(_ key: String, legacyKeys: [String] = []) -> [Double] {
        ...
        assert(retrieval != nil, "Failed to restore attribute '\(key)' to object '\(self.objectName)'")
        let result = (retrieval ?? []).compactMap(\.double)
        assert(
            retrieval?.count == result.count,
            "JSON array came with \(retrieval?.count ?? -1) elements, but only \(result.count) could be restored"
        )
    ...
    public func get(_ key: String, legacyKeys: [String] = []) -> [Bool] {
        ...
        assert(retrieval != nil, "Failed to restore attribute '\(key)' to object '\(self.objectName)'")
        let result = (retrieval ?? []).compactMap(\.bool)
        assert(
            retrieval?.count == result.count,
            "JSON array came with \(retrieval?.count ?? -1) elements, but only \(result.count) could be restored"
        )
    ...
    public func get(_ key: String, legacyKeys: [String] = []) -> [Date] {
        ...
        assert(retrieval != nil, "Failed to restore attribute '\(key)' to object '\(self.objectName)'")
        let stringResult = (retrieval ?? []).compactMap(\.string)
    ...
    public func get(_ key: String, legacyKeys: [String] = []) -> [Date?] {
        ...
        assert(retrieval != nil, "Failed to restore attribute '\(key)' to object '\(self.objectName)'")
        let stringResult = (retrieval ?? []).map(\.string)
        return stringResult.map({
            if let stringDate = $0 {
    ...
    internal func restore<T>(_ type: T.Type) -> T where T: Storable {
        ...
        guard let object = parse as? T else {
            fatalError(
                "Object \(type.self) could not be restored - some class within its inheritance tree likely forgot to add a variable within the toDataObject call"
            )
        }
        return object
    }
